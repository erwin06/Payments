...
var _d_button_ok = "Aceptar"
var _d_button_cancel = "Cancelar"
...
var Dialog = {
    ...
    block: {
        info: function(message) {
            Dialog.show(message, "alert-info")
        }
    },

    alert: {
        error: function(message, cb){
            Dialog.show(message, "alert-danger", [_d_button_ok], "btn-danger", "border: 1px solid #ff1103;background: white;color: black;" ,function(){
                Dialog.hide();
                if(cb)cb();
            })
        }
    ...
    show: function (message, divClass, buttons, buttonsClass, divStyle, cb){
        _func = cb?cb:(function() {});
        var _buttons = ""
        if (buttons) {
            var cButtons = buttons.length;
            _buttons = "<br><br>";
            for (var i = 0; i < cButtons; i++) {
                _buttons += ' <button id="btnAlert' + buttons[i] + '" onclick="_func(\'' + buttons[i] + '\')" class="btn ' + buttonsClass + '">' + buttons[i] + '</button> ';
            }
        }

        $.blockUI({
            message: '<div style="margin: auto; width: 90%; max-width: 600px; border:0; ' + (divStyle?divStyle:"") +' " class="alert ' + divClass + '"> ' + message + ' ' +
                    ' ' + _buttons + ' </div>',
            css: {backgroundColor: "transparent", border: 0, width: "100%", left: "0"}
        });
